...
@Component({
    selector: 'app-memorial-details',
    standalone: true,
    imports: [CommonModule],
    template: `
    <div class="details-block text-center">
      <h1 class="details-title">Memorial Details</h1>
      <p class="detail-line">
        <strong>Date:</strong> {{ funeralService.convertDate(data?.dateofmemorial) || 'Date: to be uploaded' }}
      </p>
      <p class="detail-line">
        <strong>Venue:</strong> {{ data?.memorialvenue || 'Venue: to be uploaded' }}
      </p>
      <p class="detail-line">
        <strong>Time:</strong> {{ data?.memorialtime || 'Time: to be uploaded' }}
      </p>
    </div>
  `,
    styles: [`
...
export class MemorialDetailsComponent {
